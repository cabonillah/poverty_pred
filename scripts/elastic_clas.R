...
wf <- workflows("elastic", "clas")
grid <- grids("elastic", "clas")
result <- wf %>% tuning("elastic", "clas", grid, validation_split)
...
report <- data.frame(
    Problema = "Clas.", Modelo = "Elastic Net",
    Penalidad = "0.000325", Mixtura = "0.3666667",
    result %>% show_best(n = 1) %>% mutate(mean = metric)
)

saveRDS(report, file = "../stores/elastic_clas.rds")
