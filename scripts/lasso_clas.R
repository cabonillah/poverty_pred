...
wf <- workflows("lasso", "clas")
grid <- grids("lasso", "clas")
result <- wf %>% tuning("lasso", "clas", grid, validation_split)
...
report <- data.frame(
    Problema = "Clas.", Modelo = "Lasso",
    Penalidad = "0.0001", Mixtura = "1",
    result %>% show_best(n = 1) %>% mutate(mean = metric)
)

saveRDS(report, file = "../stores/lasso_clas.rds")
