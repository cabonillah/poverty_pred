...
wf <- workflows("ridge", "clas")
grid <- grids("ridge", "clas")
result <- wf %>% tuning("ridge", "clas", grid, validation_split)
...
report <- data.frame(
    Problema = "Clas.", Modelo = "Ridge",
    Penalidad = "0.0001", Mixtura = "0",
    result %>% show_best(n = 1) %>% mutate(mean = metric)
)

saveRDS(report, file = "../stores/ridge_clas.rds")
